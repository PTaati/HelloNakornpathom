...
public sealed class HnpWorldGame:MonoBehaviour {
 ...
 public void SetExternalSuspended(string value){externalSuspended=string.Equals(value,"true",StringComparison.OrdinalIgnoreCase)||value=="1";if(externalSuspended)ResetInput();}
 ...
 void BuildUI(){
  ...
  menuPanel=Panel("Settings",safe,new(0,1),new(0,1),new(380,316),new(206,-290),new(.02f,.075f,.07f,.98f)).rectTransform;
  var tb=Btn("TIME","ช่วงเวลา : ยามเย็น",menuPanel,new(.5f,.5f),new(348,76),new(0,78),()=>{worldTime=Daylight>.5f?22f/24:10f/24;ApplyTime();});timeText=tb.GetComponentInChildren<Text>();
  var ab=Btn("SOUND","ดนตรี : เปิด",menuPanel,new(.5f,.5f),new(348,76),Vector2.zero,ToggleMusic);audioText=ab.GetComponentInChildren<Text>();
  Btn("RETURN","กลับสถานี",menuPanel,new(.5f,.5f),new(348,76),new(0,-78),()=>{ReturnToStation();menuOpen=false;menuPanel.gameObject.SetActive(false);});menuPanel.gameObject.SetActive(false);
 ...
 void SetMusicPaused(bool pause){if(!music)return;if(pause&&music.isPlaying)music.Pause();else if(!pause&&Started&&focused&&!mapOpen&&!menuOpen&&!externalSuspended&&!music.isPlaying)music.UnPause();}
