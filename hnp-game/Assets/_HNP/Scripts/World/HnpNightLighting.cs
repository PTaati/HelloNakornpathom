...
namespace HNP.World {
 public sealed class HnpNightLighting:MonoBehaviour {
  ...
  Material atlas,glow;Transform display;float night=-1;readonly List<Fixture> fixtures=new();readonly List<Material> monumentMaterials=new();
  ...
  void Start(){
   // Existing street-lamp heads, from the preserved town source (negative-X import).
   foreach(float x in new[]{-88f,-59,-42,-25,0})foreach(float z in new[]{-7.8f,7.8f}){
   ...
   var chedi=GameObject.Find("Phra Pathom Chedi v004")??GameObject.Find("Phra Pathom Chedi v002")??GameObject.Find("Phra Pathom Chedi");
   ...
   Vector3 C(float radius,float h,float angle)=>new(53+radius*Mathf.Cos(angle),2.9f+h,radius*Mathf.Sin(angle));
   ...
   var chediCenter=chedi.transform.position;
   Lamp("Chedi flood west",transform,chediCenter+new Vector3(-36,9,-18),chediCenter+new Vector3(0,16,0)-(chediCenter+new Vector3(-36,9,-18)),LightType.Spot,65,160,new Color(1,.65f,.28f),true,70);
   Lamp("Chedi flood east",transform,chediCenter+new Vector3(-36,12,18),chediCenter+new Vector3(0,19,0)-(chediCenter+new Vector3(-36,12,18)),LightType.Spot,65,160,new Color(1,.65f,.28f),true,70);
   Lamp("Portico flood",transform,chediCenter+new Vector3(-43,13,0),chediCenter+new Vector3(-26,11,0)-(chediCenter+new Vector3(-43,13,0)),LightType.Spot,32,80,new Color(1,.89f,.64f),true,55);
   Update();
  }
  ...
  public void LogNightDiagnostics(){var details=GameObject.Find("Temple reference details");Debug.Log("HNP008_QA "+JsonUtility.ToJson(new Snapshot{night=night,emission=atlas.GetColor("_EmissionColor").maxColorComponent,street=fixtures.Count(f=>f.light.name=="Street light"),headlights=fixtures.Count(f=>f.light.name=="Vehicle headlight"),cabin=fixtures.Count(f=>f.light.name=="Vehicle cabin light"),lit=fixtures.Count(f=>f.light.enabled),outlinedCars=cars,monumentProfile=monumentLightProfile?monumentLightProfile.name:"legacy003",templeRenderers=details?details.GetComponentsInChildren<Renderer>().Count(r=>r.enabled):0,collider=GameObject.Find("Chedi solid core").GetComponent<MeshCollider>()!=null,player=game.player.transform.position}));}
 }
}
