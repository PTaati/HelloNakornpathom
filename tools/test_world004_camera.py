...
OUT=ROOT/'reports/world'/time.strftime('world004-camera-%Y%m%d-%H%M%S');OUT.mkdir(parents=True)
class Quiet(http.server.SimpleHTTPRequestHandler):
 def log_message(self,*args):pass
server=http.server.ThreadingHTTPServer(('127.0.0.1',0),functools.partial(Quiet,directory=str(ROOT/'builds/world004/web')))
threading.Thread(target=server.serve_forever,daemon=True).start()
report={'status':'FAIL','physical_mobile':'NOT RUN','snapshots':{},'errors':[]};samples=[]
try:
 with sync_playwright() as p:
  ...
  a=snap('sky');capture('sky');assert a['pitch']<=-84 and a['forward']['y']>.99,a;assert a['camera']['y']>.25,a
  ...
  a=snap('portrait_sky');capture('portrait-sky');assert a['pitch']<=-84 and a['forward']['y']>.99,a
